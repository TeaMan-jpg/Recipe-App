...
function Recipe2({title,extendedIngredients,summary,cookingMinutes,servings,analyzedInstructions,image,sourceUrl,email}:{title:string,extendedIngredients:Ingredient[],summary:string,cookingMinutes:string,servings:Number,analyzedInstructions:Instruction[],image:string,sourceUrl:string,email:string}) {
    ...
    const handleModal = () => {
        setModalAppear(prevState => !prevState)
    }
    return (
        <>
        <div className=" h-64 mx-7 rounded-md flex flex-row" style={{width:isAppearing ? "70%" : "100%",border:"1px solid white"}}>
            ...
            <div className="h-[150px] relative overflow-hidden flex flex-col">
                ...
                <div className="flex flex-row z-10 absolute float-left left-0 top-0 bg-white text-black rounded m-1 text-center">
                        ...
                        <h6 className="mx-2 text-center">15 min</h6>
                </div>
                <div className="p-4">
                ...
                <button onClick={handleModal}>More</button>
